...
begin
  ...
  opendlg.Title := 'Загрузить сетку из файла';
  opendlg.InitialDir := GetCurrentDir;
  opendlg.Filter := 'Простая воксельная сетка (plain voxel grid)|*.pvg';
...
begin
  ...
  savedlg.Title := 'Сохранить сетку в файл';
  savedlg.InitialDir := GetCurrentDir;
  savedlg.Filter := 'Простая воксельная сетка (plain voxel grid)|*.pvg';
...
var
  ...
  x,y,z:integer;
begin
  //тут должен осушествляться выбор по типу кисти
  if rdBrushOneVox.Checked=true then
  begin
    v:=spBrushRed.Value;
...
begin
  ...
  lbBrushColor.Caption:='Красный: '+inttostr(curbrush.CR)+sLineBreak+
  'Зелёный: '+inttostr(curbrush.CG)+sLineBreak+
  'Синий: '+inttostr(curbrush.CB);
